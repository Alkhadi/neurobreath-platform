...
export type NbcardInstallState =
  | 'idle'
  | 'available'
  | 'prompting'
  | 'accepted'
...
export function useNbcardInstall() {
  const [deferredPrompt, setDeferredPrompt] = useState<BeforeInstallPromptEvent | null>(null)
  const [isStandalone, setIsStandalone] = useState(false)
  const [installState, setInstallState] = useState<NbcardInstallState>('idle')
  ...
  const triggerInstall = useCallback(async () => {
    ...
    try {
      ...
      if (outcome === 'accepted') {
        ...
        // Give the UI a chance to show an "accepted" state before settling on "installed".
