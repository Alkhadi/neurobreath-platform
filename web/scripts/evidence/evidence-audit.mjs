...
import * as evidencePage from '../../lib/evidence/page-evidence.ts';
import * as registry from '../../lib/evidence/evidence-registry.ts';
import * as contentPages from '../../lib/content/pages/index.ts';
...
const readAllFiles = (dir, extList, fileList = []) => {
...
Object.entries(evidencePage.evidenceByRoute).forEach(([route, manifest]) => {
  const routeIssues = [];
  ...
  citationIds.forEach(id => {
    if (!registry.evidenceSources[id]) {
  ...
  if (manifest.reviewedAt && manifest.reviewIntervalDays) {
    ...
    const overdueDays = Math.ceil((now - nextReview) / (1000 * 60 * 60 * 24));
...
contentPages.canonicalPages.forEach(page => {
...
const missingRoutes = [...evidenceUsage].filter(route => !evidencePage.evidenceByRoute[route]);
